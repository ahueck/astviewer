...
void QueryApp::createInputWidget() {
  ...
  auto h = new InputHighlighter(input);
  ...
  QObject::connect(this, SIGNAL(fileLoadUnlock(bool)), input, SLOT(setEnabled(bool)));
  QObject::connect(this, SIGNAL(queryUnlock(bool)), input, SLOT(setEnabled(bool)));
...
void QueryApp::createClangSession() {
  ...
  connect(clang_tool, SIGNAL(compilationDataBaseChanged(QStringList)), win, SLOT(updateDbView(QStringList)));
}
