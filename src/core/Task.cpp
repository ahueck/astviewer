...
void Task::fileLoad(Command cmd) { emit commandSkipped(cmd); }

void Task::fileStore(Command cmd) { emit commandSkipped(cmd); }

void Task::sourceSelection(Command cmd) { emit commandSkipped(cmd); }

void Task::commandInput(Command cmd) { emit commandSkipped(cmd); }

void Task::compilationDb(Command cmd) { emit commandSkipped(cmd); }
...
void Task::handleCommand(Command cmd) {
  ...
  switch (cmd.t) {
    ...
    case Command::CommandType::compilationDb:
      ...
      break;
    default:
      qDebug() << "Unsupported command type: " << cmd;
